...
// Preluăm datele actuale ale trainer-ului din tabelul traineri
$query = "SELECT * FROM traineritest WHERE id = ?";
...
if ($result && $result->num_rows > 0) {
  ...
  // Nu s-au găsit date pentru trainerul curent
  $trainer = null;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // Actualizăm datele în tabelul traineri
  if (!empty($parola)) {
    // Dacă s-a furnizat o parolă nouă, actualizăm și parola
    $update_query = "UPDATE traineritest SET nume = ?, email = ?, numartel = ?, parola = ? WHERE id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("ssssi", $nume, $email, $numartel, $parola, $trainer_id);
  } else {
    // Dacă nu s-a furnizat parolă nouă, actualizăm doar celelalte câmpuri
    $update_query = "UPDATE traineritest SET nume = ?, email = ?, numartel = ? WHERE id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("sssi", $nume, $email, $numartel, $trainer_id);
  }
...
<body>
                  ...
                  <ul class="nav navbar-nav flex-row ml-auto">
                    <li class="dropdown nav-item">
                      <a class="nav-link" href="#" data-toggle="dropdown">
                        <?php
                        if ($trainer && isset($trainer['poza']) && !empty($trainer['poza'])) {
                          echo '<img src="data:image/jpeg;base64,' . base64_encode($trainer['poza']) . '" 
                ...
                <form method="POST">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="nume"
                      value="<?php echo htmlspecialchars($trainer['nume'] ?? ''); ?>" required>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="tel" class="form-control" name="numartel"
                      value="<?php echo htmlspecialchars($trainer['numartel'] ?? ''); ?>" required>
                  </div>
  ...
  <!-- Scripts -->
  <script src="js/trainer/jquery-3.3.1.slim.min.js"></script>
  <script src="js/trainer/popper.min.js"></script>
  <script src="js/trainer/bootstrap.min.js"></script>
  <script src="js/trainer/jquery-3.3.1.min.js"></script>
