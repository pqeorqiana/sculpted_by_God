...
// Preluăm datele actuale ale user-ului din tabelul useri
$query = "SELECT * FROM useritest WHERE id = ?";
...
if ($result && $result->num_rows > 0) {
  ...
  // Nu s-au găsit date pentru userul curent
  $user = null;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  // Actualizăm datele în tabelul useri
  if (!empty($parola)) {
    // Dacă s-a furnizat o parolă nouă, actualizăm și parola
    $update_query = "UPDATE useritest SET nume = ?, email = ?, numartel = ?, parola = ? WHERE id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("ssssi", $nume, $email, $numartel, $parola, $user_id);
  } else {
    // Dacă nu s-a furnizat parolă nouă, actualizăm doar celelalte câmpuri
    $update_query = "UPDATE useritest SET nume = ?, email = ?, numartel = ? WHERE id = ?";
    $stmt = $con->prepare($update_query);
    $stmt->bind_param("sssi", $nume, $email, $numartel, $user_id);
  }
...
<body>
                  ...
                  <ul class="nav navbar-nav flex-row ml-auto">
                    <li class="dropdown nav-item">
                      <a class="nav-link" href="#" data-toggle="dropdown">
                        <?php
                        if ($user && isset($user['poza']) && !empty($user['poza'])) {
                          echo '<img src="data:image/jpeg;base64,' . base64_encode($user['poza']) . '" 
                ...
                <form method="POST">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="nume"
                      value="<?php echo htmlspecialchars($user['nume'] ?? ''); ?>" required>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="tel" class="form-control" name="numartel"
                      value="<?php echo htmlspecialchars($user['numartel'] ?? ''); ?>" required>
                  </div>
  ...
  <!-- Scripts -->
  <script src="js/user/jquery-3.3.1.slim.min.js"></script>
